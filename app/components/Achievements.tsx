...
const items = [
  ...
  },
  {
    ...
    icon: <Image src='/icons/exp.png' width={64} height={64} alt="experience" />
  },
  {
    number: '200+',
    ...
    icon: <Image src='/icons/diamond.png' width={64} height={64} alt="unique projects" />
  },
  {
    number: '100+',
    ...
    icon: <Image src='/icons/clapperboard.png' width={64} height={64} alt="Promo movies" />
  },
  {
    number: '50+',
    title: 'Współpracujących firm',
    description: 'Dzięki ponad 50 zrealizowanym współpracom z różnorodnymi firmami, zdobyliśmy cenne doświadczenie i zaufanie w branży. ',
    icon: <Image src='/icons/handshake.png' width={64} height={64} alt="collaborations" />
  }
]
  ...
  return (
    <div className='relative flex flex-col w-full px-4 sm:px-6  lg:px-8 xl:px-20 py-6 md:py-10 lg:py-12 bg-transparent  overflow-clip'>
      <div className='absolute bottom-32 right-0 h-[400px] w-[404px] z-0'>
        <Image src='/images/watercolor-splash-02.png' fill alt='watercolor-splash-01'  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className='blur-md opacity-40 '/>
      </div>
      ...
      <div className='flex flex-row flex-wrap gap-16  items-stretch xxs:gap-2 sm:gap-2 lg:gap-8 justify-center py-6 md:py-8 lg:py-16 xl:py-20 z-10'>
        {
          items.map((item, index) => (
            <div key={index} className='flex justify-center items-center px-2 py bg-white group rounded-lg hover:scale-105 transition-transform duration-300 ease-in-out border-1 border-solid border-black/20  outline outline-blue-500/50 outline-1 shadow-inner  xxs:w-[175px] xxs:min-h-[155px] hover:rotate-1 z-10' >
              <div className='flex flex-col items-center text-center xxs:gap-1 md:gap-3' >
                ...
                <h3 className='text-base lg:text-xl font-bold text-blue-800'>{item.number}</h3>
                  <h3 className='text-base lg:text-xl font-bold text-blue-800'>{item.title}</h3>
