    ...
    <div className=" w-full max-w-7xl mx-auto px-4 ">
      ...
      <div className="flex justify-end  mb-5 lsm:mb-8  ">
        <h2 className="text-4xl font-bold text-center relative inline-block  ">
          {title}
          <div className={`absolute -bottom-3 left-auto right-0 h-4  -z-10 transform -rotate-1 `} style={{ backgroundColor: accentColor, width: '100%' }}></div>
        </h2>
      </div>
        ...
        <div className="flex flex-col md:flex-row xxs:gap-4 lsm:gap-8 md:gap-12 items-center">
          ...
          <div className="w-full items-center md:w-1/2 gap-1">
            ...
            <div className="flex items-center gap-3">
              <span className={` text-6xl  lmd:text-7xl font-bold`} style={{ color: accentColor }}>{number}</span>
              <h3 className="xxs:text-xl sm:text-xl md:text-xl lmd:text-2xl font-bold ">{subtitle}</h3>
            </div>
            <p className="py-2 xxs:text-base sm:text-md lsm:text-lg text-gray-600 leading-relaxed">{description}</p>
            <Button className="bg-[#0C8FC3] hover:bg-[#0A7DAB] text-white rounded-full px-8">Więcej</Button>
          </div>
        </div>
      </div>
    </div>
